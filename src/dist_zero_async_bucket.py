...
class AsynAccumDDPWrapper(nn.Module):
        ...
        for name, params in self.model.named_parameters():
            ...
            def hook(param):
                ...
                if self.grad_update_count[self.param_mapping[param_id]] == len(param_group):
                    ...
                    h = dist.all_reduce(flat_grads,op=dist.ReduceOp.SUM, async_op=True)
                    if (flat_grads.eq(0).all()) or (flat_grads is None):
                        print(f'RANK: {self.rank} flat grads ALL ZEROS')
...
def run(rank,world_size,args):
   ...
   # bucket_index -> parameters
    ...
    # wrapper
    ddp_model = AsynAccumDDPWrapper(model, param_buckets, param_mapping,rank, world_size)
    dataset = TensorDataset(X,y)
    sampler = DistributedSampler(dataset, num_replicas=world_size, rank=rank, shuffle=True)
    dataloader = DataLoader(dataset, batch_size=args['batch_size'], sampler=sampler)
    for p in param_buckets[rank]:
        print(p.shape)
    optimizer = optim.AdamW(model.parameters(), lr=0.001)
    # optimizers = []
    # for pg in param_buckets:
    #     optimizers.append(optim.AdamW(pg, lr=0.001))
    ...
    for epoch in range(num_epochs):
        ...
        for batch_X, batch_y in dataloader:
            ...
            # for o in optimizers:
            #     o.zero_grad()
            # optimizers[dist.get_rank()].zero_grad()
            loss.backward()
            #sync 
            ddp_model.sync()
            # Update weights
